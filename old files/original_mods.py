...
from tkinter import * #El paquete tkinter («interfaz Tk») es la interfaz por defecto de Python para el kit de herramientas de GUI Tk.
from tkinter import messagebox
from functools import partial
import tkinter as tk
import pyautogui # Libreria para control de teclado y raton
import serial
import sys
import glob 
from datetime import datetime
from dotenv import load_dotenv # Sirve para leer un archivo de contraseñas
from urllib.parse import quote
from urllib.request import Request, urlopen #bot de telegram y formar http request
import json # para procesar las imagenes que se envian a telegram
import requests
import cv2
import pytesseract #Libreria de OCR
import pandas as pd
...
engine = create_engine(SQL_chain, echo=False)
...
with open(resource_path("images/basic_btn_data.csv")) as file: # carga el archivo csv donde se especifica el formato de cada boton en la interfz
	type(file)
...
def write_log(logtype,texto,ShopOrder,BoxType,StandardPack):
	...
	if file_exists == True:
		with open(ruta, "a+") as file_object: #w+, r+ o a+ – Lectura y escritura
...
class Passwordchecker(tk.Frame):
	...
	def initialize_user_interface(self):
		...
		self.parent.protocol("WM_DELETE_WINDOW", self.quit) # make the top right close button minimize the main window
		self.parent.title("Mubea de Mexico - Interfaz para impresión de etiquetas.") # titulo de la interfaz grafica
...
######Button declaration area
		...
		#process the first button
			...
			globals()[a_temp].configure(command=partial(self.Selector, int(rows[i-1][5]))) # con la funcion selector se le asigna a cada boton una funicion especifica
...
##########Selector is the function that commands buttons actions
	...
	def quit(self):
		if messagebox.askyesno('Salida','¿Seguro que quiere salir?'):
			send_message(zader,quote(f'{Line_ID}: El sistema se ha detenido'), token_Tel)
			send_message(ivan,quote(f'{Line_ID}: El sistema se ha detenido'), token_Tel)
...
if __name__ == '__main__':
	send_message(zader,quote(f'{Line_ID}: El sistema se ha iniciado'), token_Tel) # manda un mensaje de telegram al usuario notificando que ya inicio el sistema
	send_message(ivan,quote(f'{Line_ID}: El sistema se ha iniciado'), token_Tel)
	#SIGTERM signal
	finish = False
	#tkinter class assign
	root = tk.Tk() #inicia la intefaz grafica del sistema... tkinter.Tk(screenName=None, baseName=None, className='Tk', useTk=True, sync=False, use=None)
